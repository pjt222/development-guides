...
3. **Agents** (`agents/` directory): Persona definitions for Claude Code subagents. Each agent is a markdown file with YAML frontmatter (`name`, `description`, `tools`, `model`, `priority`) defining *who* handles a task. Currently 15 agents across development, compliance, review, and specialty domains.
...
- `skills/_registry.yml` is the machine-readable catalog of all 61 skills across 14 domains: r-packages (10), compliance (7), git (6), general (5), review (5), containerization (4), reporting (4), mcp-integration (3), web-dev (3), bushcraft (3), esoteric (3), defensive (3), design (3), data-serialization (2).
- `agents/_registry.yml` is the machine-readable catalog of all 15 agents.
